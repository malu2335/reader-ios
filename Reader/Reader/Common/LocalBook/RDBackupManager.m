...
+ (BOOL)writeAIConfigToZipWriter:(id)writer
{
    ...
    if (aiData.length == 0) {
        // 备份格式不变量:ai_config.json 条目始终存在(可为空 profiles),
        // 以便 finalize 自检与恢复路径稳定(Issue 13)
...
+ (void)restoreAIConfigFromZip:(id)zip
{
    ...
    NSData *aiData = [archive dataForEntry:[self aiConfigEntryName] maxUncompressedBytes:2ull * 1024 * 1024];
    if (aiData.length > 0) {
...
+ (void)createBackupWithComplete:(void(^)(NSString * _Nullable, NSString * _Nullable))complete
{
    ...
    [RDLibraryMutationCoordinator performAsync:^{
        ...
        // finalize 后 reopen 自检:必需条目(bookshelf、AI config、每本 source)必须可读(P2-CHAIN-02)
        ...
        if (![verify dataForEntry:[self aiConfigEntryName] maxUncompressedBytes:2ull * 1024 * 1024]) {
            [[NSFileManager defaultManager] removeItemAtPath:zipPath error:nil];
            finish(nil, @"备份自检失败:缺少 AI 配置条目");
            return;
        }
        for (RDBookDetailModel *book in localBooks) {
            NSString *filePath = [RDLocalBookManager absolutePathForBook:book];
...
+ (void)restoreFromURL:(NSURL *)url complete:(void(^)(NSInteger, NSString * _Nullable))complete
{
    ...
    [RDLibraryMutationCoordinator performAsync:^{
        ...
        NSData *aiProbe = [zip dataForEntry:[self aiConfigEntryName] maxUncompressedBytes:kBackupJSONConfigMax];
