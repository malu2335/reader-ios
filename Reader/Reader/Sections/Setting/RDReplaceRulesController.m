...
- (void)p_importExportMenu
{
    ...
    [RDPaperAlert showActionSheetWithTitle:@"导入/导出"
        ...
        [RDPaperAlertAction actionWithTitle:@"粘贴导入"
                                   subtitle:@"JSON 文本 / https 链接 / legado://… 自动识别"
                                      style:RDPaperAlertActionStyleDefault
                                    handler:^{
...
/// 粘贴导入:同一输入框支持 JSON 正文 / http(s) 链接 / legado:// 一键导入链接
- (void)p_importFromPaste
{
    ...
    [RDPaperAlert showTextFieldsWithTitle:@"粘贴导入"
                                  message:@"可粘贴:规则 JSON、https 直链,或 legado://import/replaceRule?src=…"
                               fieldSpecs:@[
        ...
        if (raw.length == 0) {
            [RDPaperAlert showToast:@"请粘贴 JSON 或链接"];
            return;
        }
...
- (BOOL)p_looksLikeURLOrLegadoLink:(NSString *)s
{
    NSString *t = s.lowercaseString;
    if ([t hasPrefix:@"http://"] || [t hasPrefix:@"https://"] || [t hasPrefix:@"legado://"]) {
        return YES;
    }
    // 单行且像域名路径
    if (![s containsString:@"\n"] && ![s hasPrefix:@"{"] && ![s hasPrefix:@"["]
        && ([t containsString:@".json"] || [t containsString:@"replaceRule"] || [t containsString:@"replacerule"])) {
        return YES;
    }
    return NO;
}
...
{
    if ([self p_looksLikeURLOrLegadoLink:raw]) {
        [self showLoading:@"导入中…" cancel:nil];
        __weak typeof(self) weakSelf = self;
        [[RDReplaceRuleStore sharedInstance] importFromURLString:raw
                                                           merge:YES
                                                      completion:^(NSInteger count, NSError *error) {
            [weakSelf hideLoading];
            if (error) {
                [RDPaperAlert showResultSuccess:NO title:@"导入失败" message:error.localizedDescription];
                return;
            }
            [weakSelf p_reload];
            [RDPaperAlert showResultSuccess:YES
                                      title:@"导入成功"
                                    message:[NSString stringWithFormat:@"已合并 %ld 条规则", (long)count]];
        }];
        return;
    }
